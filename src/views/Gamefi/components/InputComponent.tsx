...
import { Button, Flex, Heading, Input, Text, useModal } from '@metagg/mgg-uikit'
...
const Component: React.FC<ComponentProps> = ({
  ...
  const { userTotalStaked, userStakingBal, userAllowance } = useMemo(() => {
    return {
      userTotalStaked: userData.fixedApr
        ? getBalanceAmount(userData.fixedApr.totalStaked, currentStake.stakingToken.decimals)
        : BIG_ZERO,
      userStakingBal: userData
        ? getBalanceAmount(userData.stakingTokenBalance, currentStake.stakingToken.decimals)
        : BIG_ZERO,
      userAllowance: userData
        ? getBalanceAmount(toBigNumber(userData.allowance.toString()), currentStake.stakingToken.decimals)
        : BIG_ZERO,
    }
  ...
  const handleStakeClick = useCallback(() => {
    if (!userStakingBal.lt(new BigNumber(stakeAmount))) {
